...
export async function GET(){
 ...
 const summary=companies.map(c=>{const minutes=c.workEntries.reduce((s,e)=>s+e.minutes,0);const travel=c.workEntries.reduce((s,e)=>s+(e.travelMinutes||0),0);const extraWorkCosts=c.workEntries.reduce((s,e)=>s+Number(e.additionalCost||0),0);const net=Number(c.netAmount||0),costs=Number(c.travelCost||0)+Number(c.extraCost||0)+extraWorkCosts;return{name:c.name,employee:c.assignedUser?.name,netAmount:net,minutes,travelMinutes:travel,costs,hourlyRate:minutes?(net-costs)/(minutes/60):0}}).sort((a,b)=>a.hourlyRate-b.hourlyRate);
