...
export async function GET(){
 ...
 const summary=companies.map(c=>{const minutes=c.workEntries.reduce((s,e)=>s+e.minutes,0);const travel=c.workEntries.reduce((s,e)=>s+(e.travelMinutes||0),0);const extraWorkCosts=c.workEntries.reduce((s,e)=>s+Number(e.additionalCost||0),0);const ordersNet=c.extraOrders.reduce((s,o)=>s+Number(o.netAmount||0),0);const orderCosts=c.extraOrders.reduce((s,o)=>s+Number(o.travelCost||0)+Number(o.extraCost||0),0);const net=Number(c.netAmount||0)+ordersNet,costs=Number(c.travelCost||0)+Number(c.extraCost||0)+extraWorkCosts+orderCosts;const timeCost=(minutes/60)*120;return{name:c.name,employee:c.assignedUser?.name,monthlyAmount:Number(c.netAmount||0),extraOrdersNet:ordersNet,totalNet:net,minutes,travelMinutes:travel,costs,timeCost,profit:net-costs-timeCost,hourlyRate:minutes?(net-costs-timeCost)/(minutes/60):0}}).sort((a,b)=>a.profit-b.profit);
