...
export async function GET(){
 ...
 h.columns=[{header:'Data',key:'date',width:16},{header:'Firma',key:'company',width:30},{header:'Użytkownik',key:'user',width:22},{header:'Czynność',key:'title',width:24},{header:'Opis',key:'description',width:50},{header:'Czas pracy',key:'time',width:14},{header:'Dojazd',key:'travel',width:14}];
 entries.forEach(e=>h.addRow({date:e.date.toISOString().slice(0,10),company:e.company.name,user:e.user.name,title:e.title,description:e.description,time:minToText(e.minutes),travel:minToText(e.travelMinutes||0)}));
 h.getRow(1).font={bold:true,color:{argb:'FFFFFFFF'}};h.getRow(1).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFFF5A14'}};h.autoFilter='A1:G1';
 const st=wb.addWorksheet('Statystyki');st.addRows([['Statystyka','Wartość'],['Liczba firm',companies.length],['Liczba wpisów',entries.length],['Suma godzin',entries.reduce((x,e)=>x+e.minutes,0)/60],['Suma dojazdów',entries.reduce((x,e)=>x+(e.travelMinutes||0),0)/60]]);st.getRow(1).font={bold:true,color:{argb:'FFFFFFFF'}};st.getRow(1).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF132734'}};
