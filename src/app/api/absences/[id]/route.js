...
export async function PUT(req,{params}){
  ...
  if(user.role!=='ADMIN'&&current.userId!==user.id)return Response.json({error:'Brak uprawnień.'},{status:403});
  try{
    ...
    if(body.minutes!==undefined)data.minutes=Math.max(1,Math.min(450,Number(body.minutes||450)));
